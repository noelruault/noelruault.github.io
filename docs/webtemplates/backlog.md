...
- [x] `t50-likova` [d5] HUGE: same as t10 for https://likova.space into `templates/likova/` (prefix `lk-`). Acceptance: same as t10. SPLIT — failed green gate whole, replaced by `t50a-likova-structure` + `t50b-likova-motion`.
- [ ] `t50a-likova-structure` [d5]: clean-room deep-copy https://likova.space into `templates/likova/` (prefix `lk-`) per spec.md method, clean-room rules and modularity contract. Full top-to-bottom section inventory, `config.json` + `shell.mjs` + `components/*.mjs` for every section (modularity contract, no hand-authored HTML), responsive at 390/768/1440. Effects/motion may be stubbed or omitted here, deferred to `t50b-likova-motion`. Acceptance: gate green (manifest, render-drift, forbidden-identity incl. `likova`, shingle scan 0) with `likova` in manifest; structural side-by-side (section order + grid geometry, not motion) recorded in review.md at the three widths.
- [ ] `t50b-likova-motion` [d5] (needs: `t50a-likova-structure`): implement every effect observed on the live reference (reveal-on-scroll, hover states, any carousel/parallax/accordion) per spec.md method steps 6-7, reveal shipped with the three-guard pattern (IO + no-IO fallback + reduced-motion + noscript). Acceptance: gate green; full side-by-side parity verdict at 390/768/1440 plus effects inventory recorded in review.md, matching the bar t10/t20/t30/t40 set.
...
- [ ] `t57-charm-land` [d5] HUGE: same as t10 for https://charm.land/ into `templates/charm-land/` (prefix `ch-`); add `charm.land` to the forbidden-identity list in check.mjs matching the literal domain, never the bare word "charm" (false positives). Acceptance: same as t10.
